(function () {
    ...
    var promptOrganization = LRObject.util.getQueryParameterByName("promptOrganization");
    ...
    if(promptOrganization){
        LRObject.options.promptOrganization = true;
    ...
    if (window.lr_raas_settings.forgotpassword) {
        option.onSuccess = function (response) {
            if (!LRObject.util.federationCallbackHandler(response)) {
                window.lr_raas_settings.forgotpassword.success(response);
            }
        };
    ...
    if (window.location.search.indexOf('vtoken') > -1 && window.location.search.indexOf('vtype=reset') > -1) {
        if (window.lr_raas_settings.resetpassword) {
            option.onSuccess = function (response) {
                if (!LRObject.util.federationCallbackHandler(response)) {
                    window.lr_raas_settings.resetpassword.success(response);
                }
            };

            window.lr_raas_settings.resetpassword.success;
    ...
    var o_options = {};
    o_options.onSuccess = function(){
        $('.lr-register-link').hide();

}
o_options.onError = function(errors) {
//On Errors
console.log(errors);
};
o_options.container = "organization-container";


LRObject.init('organization',o_options);
})();
